...
Veiem com el model ha encertat el 83,33% dels registres, el mateix percentatge que el model de regressió de l'apartat anterior. Ara bé, en aquest l'especificitat ens indica que hem encertat el 93,94% dels *outputs* positius (és a dir, de diagnòstics positius), el que representa una millora respecte al 87,88% del model anterior.
...
El nostre objectiu era poder predir el diagnòstic final d'un pacient a partir de la resta de variables. Per aquesta raó s'han fet dos models predictius, un de regressió logística i un arbre de decisió, els quals han donat exactament la mateixa precisió (un 83,33%).\
\
Tot i això, l'arbre ha presentat una major sensitivitat que el model de regressió (un 93,94% en contra d'un 87,88%), i per això podem afirmar que és millor model. S'ha de tenir present que en aquest cas ens interessa predir el màxim de positius possibles, per així poder tractar al pacient en conseqüència, tot i l'increment que això suposi de falsos positius, els quals en aquest cas tampoc "no farien mal".\
\
